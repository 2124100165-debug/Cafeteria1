                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex justify-content-center gap-2">
                                {{-- Botón Ver --}}
                                <a href="{{ route('cliente.mostrar', $cliente->id_cliente) }}"
                                   class="btn btn-sm btn-info btn-uniforme">
                                    Ver
                                </a>

                                {{-- Botón Editar --}}
                                <a href="{{ route('cliente.edit', $cliente->id_cliente) }}"
                                   class="btn btn-sm btn-outline-warning btn-uniforme">
                                    Editar
                                </a>

                                {{-- Formulario Borrado Lógico --}}
                                <form action="{{ route('cliente.eliminarLog', $cliente->id_cliente) }}"
                                      method="POST"
                                      onsubmit="return confirm('¿Desactivar a este cliente?');">
                                    @csrf
                                    <button type="submit"
                                            class="btn btn-sm btn-outline-danger btn-uniforme">
                                        Eliminar
                                    </button>
                                </form>
                            </div>
