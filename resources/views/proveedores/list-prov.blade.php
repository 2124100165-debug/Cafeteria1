...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="text-dorado-kraneo mb-0">Proveedores / Lista General</h2>
        <a href="{{ route('proveedor.crear') }}" class="btn btn-warning fw-bold text-black">
            + Nuevo Proveedor
        </a>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <div class="input-group">
                ...
                <input type="text" id="buscarProveedor" class="form-control bg-dark text-white border-dorado-kraneo shadow-none" placeholder="Buscar por nombre, empresa...">
            </div>
        </div>
    </div>
        ...
        <table class="table table-dark table-striped table-hover align-middle mb-0">
            ...
            <tbody>
                @forelse($proveedores as $proveedor)
                    <tr>
                        <td class="text-dorado-kraneo fw-bold">#{{ $proveedor->id_proveedor }}</td>
                        <td class="fw-bold">{{ $proveedor->nombre }}</td>
                        <td class="text-white-50">{{ $proveedor->contacto }}</td>
                        <td class="text-warning">{{ $proveedor->telefono }}</td>
                        <td class="text-white-50">{{ $proveedor->empresa }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="text-center py-5 text-muted">
                            <p class="mb-0 fs-5">No hay proveedores registrados.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
